              ...
              <ul class="nav navbar-nav">
                ...
                <?php if (login_check($mysqli) == true) {
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            <?php
                                echo $combo->getLinkProject($_SESSION['email']);// getListProjectU($mysqli, $_SESSION['email']);
                            ?>
                        </ul>
                    </li>

                    <?php if ($_SESSION['type'] != 1) {
                        $admin_st = $_SESSION['type'];
                        $user_st = "<span style='color: black'>0</span>";
                        if ($combo->getPendingAssignment($_SESSION['id_user'], $admin_st) != "0"){
                            $user_st = "<span style='color: red'>" . htmlentities($combo->getPendingAssignment($_SESSION['id_user'], $admin_st)) . "</span>";
                        }
                        ?>
                        <li class="dropdown">
                            <button type="button" class="btn btn-default btn-xs" title="">
                                <?php echo $user_st; ?>&nbsp;&nbsp;<span class="glyphicon glyphicon-user"></span>
                            </button>
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="index.php?controller=assign&action=index&param=id_<?php echo htmlentities($_SESSION['id_user']); ?>_type_<?php echo htmlentities($_SESSION['type']); ?>"><?php echo _("Users pending assignment");?></a>
