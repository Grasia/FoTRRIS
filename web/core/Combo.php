...
class Combo {
    ...
    public function getPerfiles($selected_value=null, $blank_opt=true,$status=true) {
        ...
        try {
            if ($comboPerfil = $this->db->prepare("SELECT id, name, type FROM perfil WHERE sn_active = ? order by id")) {
     ...
     * @return string
     */
    public function getPendingAssignment($id_user, $admin) {
        $desc = "";
        if ($id_user != '') {
            $query = "SELECT count(*) as cuenta FROM arena_manager as am, project as pr, assigned_project_user as ap WHERE am.id_arena = pr.id_arena AND pr.id = ap.id_project AND am.id_user = ? AND am.sn_active = 'S' AND pr.sn_active = 'S' AND ap.sn_active = 'N';";
            $query_admin = "SELECT count(*) as cuenta FROM assigned_project_user WHERE sn_active = 'N';";

            if($admin == "2") {
                $query = $query_admin;
            }
            if ($listu = $this->db->prepare($query)) {
                if($admin == "3") {
                    $listu->bind_param('s', $id_user);  // Bind "$email" to parameter.
                }
                $listu->execute();    // Execute the prepared query.
                $listu->bind_result($peding);
                while ($listu->fetch()) {
                    $desc .= "" . $peding . "";
                }
                $listu->close();
